...
const UploadDropzone = () => {
    ...
    const [uploadingProgress, setIsUploadingProgress] = useState<number>(0);

    const { startUpload } = useUploadThing("pdfUploader");

    const { mutate: startPolling } = trpc.getFile.useMutation({
        onSuccess: (file) => {
            router.push(`/dashboard/talkpdf/${file?.id}`)
        },
        retry: true,
        retryDelay: 500
    });
    ...
    const startSimulatingProgress = () => {
        ...
        const interval = setInterval(() => {
            setIsUploadingProgress((prevProgress: any) => {
                if (prevProgress >= 95) {
                    ...
                    <div className='flex items-center justify-center h-full w-full'>
                        <label htmlFor='dropzone-file' className='flex flex-col items-center justify-center w-full h-full rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100'>
                            ...
                            {acceptedFiles && acceptedFiles[0] ? (
                                <div className='max-w-xs bg-white flex items-center rounded-md overflow-hidden outline outline-[1px] ountline-zinc-200 divide-x divide-zinc-200'>
                            ...
                            {isUploading ? (
                                <div className='w-full mt-4 max-w-xs mx-auto '>
                                    <Progress indicatorColor={uploadingProgress === 100 ? 'bg-green-500' : ''} value={uploadingProgress} className='h-1 w-full bg-zinc-200' />
